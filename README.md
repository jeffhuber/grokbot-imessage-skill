...
| `chat.db size ... exceeds snapshot limit` | Database too large for in-memory snapshot | Set `IMESSAGE_SNAPSHOT_MAX_MB` to a higher value (default 500) or archive old messages. Edit `~/Library/LaunchAgents/com.jeffhuber.grokbot-imessage.plist` and add `<key>IMESSAGE_SNAPSHOT_MAX_MB</key><string>1024</string>` under `EnvironmentVariables`, then reload the agent with `launchctl unload` and `launchctl load` |
